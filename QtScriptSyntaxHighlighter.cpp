...
// based on QScriptSyntaxHighlighter from Qt
...
namespace Evernus
{
    namespace
    {
        ...
        bool isKeyword(const QString &word)
        {
            ...
            const char * const *end = &keywords[sizeof(keywords) / sizeof(keywords[0]) - 1];
    ...
    QtScriptSyntaxHighlighter::QtScriptSyntaxHighlighter(QObject *parent)
        : QSyntaxHighlighter{parent}
    {
        mFormats[ScriptNumberFormat].setForeground(Qt::darkBlue);
        mFormats[ScriptStringFormat].setForeground(Qt::darkGreen);
        mFormats[ScriptTypeFormat].setForeground(Qt::darkMagenta);
        mFormats[ScriptKeywordFormat].setForeground(Qt::darkYellow);
        mFormats[ScriptPreprocessorFormat].setForeground(Qt::darkBlue);
        mFormats[ScriptLabelFormat].setForeground(Qt::darkRed);
        mFormats[ScriptCommentFormat].setForeground(Qt::darkGreen);
        mFormats[ScriptCommentFormat].setFontItalic(true);
    }

    void QtScriptSyntaxHighlighter::highlightBlock(const QString &text)
    {
        // states
    ...
    //                        parentheses.push_back(Parenthesis(Parenthesis::Closed, c, i));
                                      ...
                                      if (nextChar != colon)
                                          for (int j = 0; j < i; ++j) {
                                              if (format(j) == emptyFormat )
                                                  setFormat(j, 1, mFormats[ScriptLabelFormat]);
                                          }
                                  }
                              } break;
                    default:
            ...
            switch (state) {
                ...
                case StateCCommentStart2:
                                    setFormat(i - 1, 2, mFormats[ScriptCommentFormat]);
                                    makeLastStandard = false;
    //                                parentheses.push_back(Parenthesis(Parenthesis::Opened, QLatin1Char('/'), i-1));
                ...
                case StateCComment:
                                    if (makeLastStandard)
                                        setFormat(i - 1, 1, emptyFormat);
                                    makeLastStandard = false;
                                    setFormat(i, 1, mFormats[ScriptCommentFormat]);
                                    buffer = QString::null;
                                    break;
    ...
    //                                parentheses.push_back(Parenthesis(Parenthesis::Closed, QLatin1Char('/'), i));
                ...
                case StateString:
                                    if (makeLastStandard)
                                        setFormat(i - 1, 1, emptyFormat);
                                    makeLastStandard = false;
                                    setFormat(i, 1, mFormats[ScriptStringFormat]);
                                    buffer = QString::null;
                                    break;
                case StateStringEnd:
                                    if (makeLastStandard)
                ...
                case StateNumber:
                                    if (makeLastStandard)
                                        setFormat(i - 1, 1, emptyFormat);
                                    makeLastStandard = false;
                                    setFormat(i, 1, mFormats[ScriptNumberFormat]);
                                    buffer = QString::null;
                                    break;
                case StatePreProcessor:
                                    if (makeLastStandard)
                                        setFormat(i - 1, 1, emptyFormat);
                                    makeLastStandard = false;
                                    setFormat(i, 1, mFormats[ScriptPreprocessorFormat]);
                                    buffer = QString::null;
                                    break;
            }
    ...
    void QtScriptSyntaxHighlighter::highlightWord(int currentPos, const QString &buffer)
    {
        if (buffer.isEmpty())
            return;
        ...
        if (buffer.length() > 1
            ...
            setFormat(currentPos - buffer.length(), buffer.length(), mFormats[ScriptTypeFormat]);
        } else {
            if (isKeyword(buffer))
                setFormat(currentPos - buffer.length(), buffer.length(), mFormats[ScriptKeywordFormat]);
        }
    }
}

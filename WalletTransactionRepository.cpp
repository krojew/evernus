...
namespace Evernus
{
    ...
    void WalletTransactionRepository::create(const Repository<Character> &characterRepo) const
    {
        ...
        exec(QString{"CREATE INDEX IF NOT EXISTS %1_ignored_timestamp ON %1(ignored, timestamp)"}.arg(getTableName()));
        exec(QString{"CREATE INDEX IF NOT EXISTS %1_character_ignored_timestamp ON %1(character_id, ignored, timestamp)"}.arg(getTableName()));
        ...
        try
        {
            ...
            exec(QString{"CREATE INDEX IF NOT EXISTS %1_corporation_ignored_timestamp ON %1(corporation_id, ignored, timestamp)"}.arg(getTableName()));
        }
